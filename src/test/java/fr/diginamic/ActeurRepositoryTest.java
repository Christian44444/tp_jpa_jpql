...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParIdentite() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a WHERE a.identite Like :ident", Acteur.class);
		query.setParameter("ident", "Marion COTILLARD");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParAnneeNaissance() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a WHERE YEAR(a.anniversaire) = ?1 ", Acteur.class);
		query.setParameter(1, 1985);
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r WHERE r.nom LIKE :name ", Acteur.class);
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r JOIN r.film f WHERE f.annee = ?1 ", Acteur.class);
		query.setParameter(1, 2015);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays p WHERE f.annee = '2017' AND p.nom = 'FRANCE'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.realisateurs r WHERE r.identite = 'Ridley Scott' AND f.annee BETWEEN ?1 AND ?2 ", Acteur.class);
